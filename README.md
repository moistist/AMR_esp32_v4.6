...
**问题**：`readSensorData()` 中使用 `buf[6] >> 4` 提取 X 轴低 4 位，但根据 MMC5603NJ 数据手册，寄存器 0x06 (Xout2) 存放的是 Xout[3:0]，有效数据在**低 4 位**。
...
> 同理，Y 轴和 Z 轴的 `buf[7]` 和 `buf[8]` 也应使用 `& 0x0F` 而非 `>> 4`。

#### 2. ADC 等待时间可能不足

**问题**：当前等待时间为 2000µs，但 BW=11 时三轴顺序测量约需 3.6ms+，加上 Auto SR 的 SET 操作时间（375ns 脉冲 + 1ms 等待），总时间约 5ms+。

```cpp
// ⚠️ 当前值
if (micros() - t_start > 2000)  // 可能不够

// ✅ 建议修改
if (micros() - t_start > 6000)  // 留足余量
```
